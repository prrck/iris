...
Mesh::Mesh(const aiMesh &ai_mesh, const Material *material)
    : Primitive(material) {
  spdlog::info("  Loading mesh \"{}\"...", ai_mesh.mName.C_Str());
...
Mesh::Mesh(Material const *material) : Primitive(material) {
  _vertices.resize(3);
  _normals.resize(1);
  _triangles.resize(1);
...
float Mesh::triangle_intersect(Ray &ray, const IndexedTriangle &tri,
                               Vec3 &inter_point, Vec3 &normal) const {
  // MOLLER_TRUMBORE
...
float Mesh::triangle_intersect_t(const Ray &ray,
...
std::optional<Intersection> Mesh::intersect(Ray &ray) const {
  ...
  for (auto tri = _triangles.begin(); tri != _triangles.end(); ++tri) {
    ...
    if (t) {
      inters.push_back(Intersection(t, inter_point, normal, this->material));
      ray.t_max = t;
    }
  }
  // }
...
// {
//     // TODO: This is just a hack for now. All points are the center of
//     bounding
//     // box.
...
Bounds3 Mesh::bounds() const { return _bounds; }